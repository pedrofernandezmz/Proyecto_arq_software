...
function Cart(){
  ...
  const renderEmptyCart = (
    <a className="empty-cart"> INICIA SESION </a>
  )

  const renderOrderButton = (
    <div className="emptySpace">
      <span> Total: ${total} </span>
      <button onClick={() => buy(cartProducts)}>FINALIZAR COMPRA</button>
    </div>
  )
  ...
  return (
    <div className="cart">
      ...
      <div id="main">
        ...
        {Cookie.get("cart") ? (Cookie.get("user_id") > -1 ? showProducts(cartProducts) : renderEmptyCart) : <a> error </a>}
